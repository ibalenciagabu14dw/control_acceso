...
router.post('/modificarHorarioProfesor', function(req, res, next) {
    ...
    horario_profesor.buscarHorarioProfesorPorId(req.query.id_horario_profesor,function(error,row){
        if(error){
            ...
            if(row.length==0){
                res.send('no hay ningun horario_profesor con ese id');
            }else{
                var dia_semana_antiguo=row[0].dia_semana;
                var hora_inicio_antiguo=row[0].hora_inicio;
                var hora_final_antiguo=row[0].hora_final;
                var id_profesor_antiguo=row[0].id_profesor;
                profesor.buscarProfesorPorId(req.query.id_profesor,function(error,row){
                    if(error){
                        res.send(error);
                    }else{
                        if(row.length==0){
                            res.send('no hay ningun profesor conese id');
                        }else{
                            horario_grupo.buscarHorarioGrupoPorId(req.query.id_horario_grupo,function(error,row){
                                if(error){
                                    res.send(error)
                                    ...
                                    if(row.length==0){
                                        res.send('no hay ningun horario_grupo con ese id');
                                    }else{
                                        horario_profesor.buscarHorarioProfesorExistente(req.query.dia_semana,req.query.hora_inicio,req.query.hora_final,req.query.id_profesor,function(error,row){
                                            if(error){
                                                res.send(error);
                                            }else{
                                                if((row.length>0)&&((dia_semana_antiguo!=req.query.dia_semana)||(hora_inicio_antiguo!=req.query.hora_inicio)||(hora_final_antiguo!=req.query.hora_final)||(id_profesor_antiguo!=req.query.id_profesor))){
                                                    res.send('ese horario profesor ya existe');
                                                }else{
                                                    horario_profesor.modificarHorarioProfesor(req.query.id_horario_profesor,req.query.dia_semana,req.query.hora_inicio,req.query.hora_final,req.query.id_profesor,function(error,row){
                                                        if(error){
                                                            res.send(error);
                                                        }else{
                                                            res.send('horario_grupo modificado correctamente');
                                                        }//else
